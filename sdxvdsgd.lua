...
do
    ...
    speedButton.Name = "SpeedButton" -- Для ідентифікації
    ...
    local longPressTime = 0.5 -- секунди для довгого натискання
    ...
    -- Обробник звичайного натискання (короткого)
    ...
    local function handleLongPress()
    ...
    -- Обробники подій для сенсорного керування
    if touchEnabled then
        speedButton.TouchLongPress:Connect(handleLongPress)
        speedButton.TouchTap:Connect(handleTap)
        
        -- Додатковий обробник для звичайних кліків (якщо це не сенсорний пристрій)
        speedButton.MouseButton1Click:Connect(handleTap)
    else
        -- Стандартне керування для ПК
        speedButton.MouseButton1Click:Connect(handleTap)
        speedButton.MouseButton2Click:Connect(handleLongPress)
    end
    ...
    -- Автоматичне закриття поля введення при кліку поза ним
    game:GetService("UserInputService").InputBegan:Connect(function(input, processed)
        if not processed and speedInput.Visible then
            if input.UserInputType == Enum.UserInputType.MouseButton1 or 
               input.UserInputType == Enum.UserInputType.Touch then
                local absPos = input.Position
                local relPos = speedInput.AbsolutePosition
                local size = speedInput.AbsoluteSize
                
                if absPos.X < relPos.X or absPos.X > relPos.X + size.X or
                   absPos.Y < relPos.Y or absPos.Y > relPos.Y + size.Y then
                    speedInput.Visible = false
                end
            end
        end
